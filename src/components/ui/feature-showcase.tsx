    ...
    <section className={cn("w-full bg-background text-foreground", className)}>
      <div className="container mx-auto grid max-w-7xl grid-cols-1 gap-10 px-6 py-16 md:grid-cols-12 md:py-20 lg:gap-14">
        ...
        <div className="md:col-span-6">
          ...
          <h2 className="text-balance text-4xl font-bold leading-[0.95] sm:text-5xl md:text-6xl text-black dark:text-white">
            {title}
          </h2>
          ...
          <div className="mt-10 max-w-xl">
            <Accordion type="single" collapsible className="w-full">
              {steps.map((step) => (
                <AccordionItem key={step.id} value={step.id}>
                  <AccordionTrigger className="text-left text-base font-medium text-black dark:text-white hover:text-gray-600 dark:hover:text-gray-300">
                    {step.title}
                  </AccordionTrigger>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              ...
              <Button
                ...
                size="lg"
                variant="secondary"
                className="border border-border bg-transparent hover:bg-white/10"
              >
                ...
                <TabsList className="flex gap-2 rounded-xl border border-border bg-background/80 p-1 backdrop-blur supports-[backdrop-filter]:bg-background/70">
                  {tabs.map((t) => (
                    <TabsTrigger
                      key={t.value}
                      value={t.value}
                      className="rounded-lg px-4 py-2 data-[state=active]:bg-foreground data-[state=active]:text-background cursor-pointer"
                    >
                      {t.label}
                    </TabsTrigger>
                  ))}
